<template>
  ...
  <c-container bg-white>
    ...
    <view class="relative z-10 bg-white p-x-[28rpx] p-b-[28rpx]">
      ...
      <!-- 空状态 -->
      <view v-if="orderList.length === 0" class="text-center text-[28rpx] text-[#999]">
        暂无委托订单
      </view>
    </view>
    ...
    >
      ...
      <view class="p-[28rpx]">
        ...
        <view v-if="makeType === 'LIMIT'" class="mb-[20rpx] flex items-center justify-between">
...
const selectMakeType = (type: 'LIMIT' | 'MARKET') => {
  ...
  if (type === 'MARKET') {
    ...
    selectedPrice.value = '';
    // 市价委托时，价格设为当前价格，但不显示
    buyOrSellPrice.value = currentPrice.value.toString();
    // 更新交易信息
    updateTradeInfo();
  }
  else {
    // 切换回限价委托时，根据当前买卖方向设置默认价格
    if (activeTab.value === 'BUY' && buyOrders.value.length > 0) {
      buyOrSellPrice.value = buyOrders.value[0].price;
    }
    else if (activeTab.value === 'SELL' && sellOrders.value.length > 0) {
      buyOrSellPrice.value = sellOrders.value[0].price;
    }
    // 更新交易信息
    updateTradeInfo();
